...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('star-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('star-tabs-nav-item', {
  ...
  return (
    <div className={`star-tabs ${className}`}>
      <ul className={navClass}>{renderNavLinks()}</ul>
      <div className="star-tabs-content">{renderContent()}</div>
    </div>
  )
}
